...
byte font[95][5] = {            /* 5x7 ASCII character font */
	...
	void GLCD_Remove_Dot(unsigned char xx,unsigned char y)
	{
		unsigned char x, i;
	...
	// 배를 그리기 위한 함수
	void GLCD_Ship(unsigned char xx, unsigned char y)
	{
		unsigned char x, i, j;
		...
		if(i == 0)      { i = 0b00000111; }
		else if(i == 1) { i = 0b00001110; }
		else if(i == 2) { i = 0b00011100; }
		else if(i == 3) { i = 0b00111000; }
		else if(i == 4) { i = 0b01110000; }
		else if(i == 5) { i = 0b11100000; }
		else if(i == 6) { i = 0b11000000; 
		                  j = 0b00000001; }
		else            { i = 0b10000000; 
		                  j = 0b00000011; }
		...
		if(y <= 63) {
			...
			ScreenBuffer[x+1][y] = j;
			GLCD_Axis_xy(x+1, y);
			datal(ScreenBuffer[x+1][y]);
		}
		else{
			ScreenBuffer[x][y] = i;
			GLCD_Axis_xy(x, y);
	...
	void GLCD_Remove_Line(unsigned char x1,unsigned char y1,unsigned char x2,unsigned char y2)
	{
		int x, y;
		if(y1 != y2)    // if y1 != y2, y is variable
		{
		...
		{
			GLCD_Remove_Dot(x1,y1);  //             it is a dot
		}
	}
	...
	void GLCD_Remove_Circle(unsigned char x1,unsigned char y1,unsigned char r)
	{
		int x, y;
		float s;
		for(y = y1 - r*3/4; y <= y1 + r*3/4; y++) // draw with y variable
